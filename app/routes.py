...
def create():
    ...
    if user.coins < 120: return "Not enough coins"
...
def renew(vps_id):
    ...
    if user.coins < 70: return "Not enough coins to renew"
...
@login_required
    ...
    expired = VPS.query.filter(VPS.expires_at < now).all()
        for v in expired:
            try: os.system(f"rm -rf vps/{v.name}")
            except: pass db.session.delete(v) db.session.commit()
...
@login_required
    ...
    if vps.type == "python"
    else ["node", f"vps/{vps.name}/index.js"]
...
@login_required
    def logs(vps_id):
        ...
        if not os.path.exists(logfile):
            return "No logs yet." with open(logfile, "r") as f:
            content = f.read()
...
@app.route("/afk")
    @login_required
        def afk():
        user = User.query.get(session["user_id"])
        user.coins += 3 db.session.commit()
